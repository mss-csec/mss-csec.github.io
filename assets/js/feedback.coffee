...
window.addEventListener "keydown", (e) ->
  if e.target.tagName == "INPUT" or e.target.tagName == "TEXTAREA" or e.target.hasAttribute('contenteditable')
    return null

  frustrationCount += 1
  if frustrationCount > 4 and document.querySelectorAll('#feedback-modal').length == 0
